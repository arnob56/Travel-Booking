...
class Bus(models.Model):
    ...
    def __str__(self):
        return f"Bus ID: {self.bus_id} | Bus Name: {self.bus_name} | From: {self.departure_location} | To: {self.destination_location} | Bus Type: {self.bus_type} | Fare: {self.fare} | Total Seats: {self.total_seats} | Available Seats: {self.available_seats}"
...
class TripHistory(models.Model):
    ...
    def __str__(self):
        return f"Trip from {self.bus.departure_location} to {self.bus.destination_location} on {self.journey_date} - {self.status}"
...
class Air(models.Model):
    ...
    def __str__(self):
        return f"Plane ID: {self.plane_id} | Plane Name: {self.plane_name} | From: {self.departure_airport} | To: {self.destination_airport} | Fare: {self.p_fare} | Total Seats: {self.p_total_seats} | Available Seats: {self.p_available_seats}"
...
class AirBooking(models.Model):
    ...
    def __str__(self):
        return f"Booking {self.plane_book_id} for {self.user.username} - {self.air.plane_name}"
...
class Events(models.Model):
    ...
    def __str__(self):
        return f"Event ID: {self.event_id} | Event Name: {self.event_name} | Location: {self.event_location} | Price: {self.price} | Available Seats: {self.available_seats}"

class EventBooking(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    event = models.ForeignKey(Events, on_delete=models.CASCADE)
    num_tickets = models.PositiveIntegerField()  # Number of tickets booked
    total_price = models.IntegerField()  # Total price for the booking
    booking_date = models.DateField(auto_now_add=True)  # Date of booking

    def __str__(self):
        return f"Booking for {self.user.username} - {self.event.event_name} | Tickets: {self.num_tickets} | Total Price: {self.total_price}"

    def can_book(self):
        return self.num_tickets <= self.event.available_seats

    def update_event_seats(self):
        if self.can_book():
    ...
    def cancel_booking(self):
        self.event.available_seats += self.num_tickets  # Restore the seats
        self.event.save()
        self.delete()  # Delete the booking record
        return True
